...
class AvaFeatureExtractor(object):
  ...
  def extract_query_features(self, preprocessed_inputs, scope):
    """Extracts first stage RPN features.
    ...
    """
    with tf.variable_scope(scope, values=[preprocessed_inputs]):
      return self._extract_query_features(preprocessed_inputs, scope)
...
class AvaMetaArch(model.DetectionModel):
  ...
  def __init__(self,
    ...
    # in the future.
    super(AvaMetaArch, self).__init__(num_classes=num_classes)
  ...
  def preprocess(self, inputs):
    ...
    """
    if inputs.dtype is not tf.uint8:
      raise ValueError('`preprocess` expects a tf.uint tensor')
  ...
  def predict(self, features):
    ...
    (rpn_box_predictor_features, rpn_features_to_crop, anchors_boxlist,
     image_shape) = self._extract_rpn_feature_maps(features['ref'])
  ...
  def _predict_second_stage(self, rpn_box_encodings,
    ...
    proposal_mean = tf.reduce_mean(flattened_proposal_feature_maps, axis=[1, 2])
    proposal_mean = tf.split(proposal_mean, len(query_box))
    weights = []
    with tf.variable_scope('att', reuse=tf.AUTO_REUSE):
      for query, ref in zip(query_proposal_feat, proposal_mean):
        query = tf.reduce_mean(query, axis=[1, 2])
        p_query = slim.fully_connected(query, 128, activation_fn=None, scope='fc_query')
        p_ref = slim.fully_connected(ref, 128, activation_fn=None, scope='fc_ref')
        p_query = tf.expand_dims(p_query, axis=0)
        p_ref = tf.expand_dims(p_ref, axis=1)
        g = tf.tanh(p_query + p_ref)
    ...
    weighted_query = []
    for feat, w in zip(query_proposal_feat, weights):
      feat = tf.layers.flatten(feat)
      feat = tf.matmul(w, feat)
      feat = tf.reshape(feat, [-1, 7, 7, 1024])
      weighted_query.append(feat)

    weighted_query = tf.concat(weighted_query, axis=0)
    flattened_proposal_feature_maps = tf.concat([flattened_proposal_feature_maps, weighted_query], axis=-1)
      ...
      if prediction_masks is not None:
        ...
        second_stage_mask_losses = ops.reduce_sum_trailing_dimensions(
          ...
          ndims=2) / (
                                     ...
                                     tf.reduce_sum(
                                       batch_mask_target_weights, axis=1,
                                       keep_dims=True
